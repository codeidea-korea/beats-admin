...
;(function($, window, undefined){
	...
	$(document).ready(function()
	{
		...
		public_vars.$sidebarMainMenu      = public_vars.$pageContainer.find('.sidebar-menu.nt_sidebar-main');	
...
{
	//hj//
	if($li.data('is-busy') || ($li.parent('.main-menu').length && public_vars.$sidebarMainMenu.hasClass('collapsed') && !($li.parent('.main-menu').closest('.sidebar-menu').hasClass('nt_sidebar-pj'))))
		return;
	...
	if($li.find('a[data-toggle="nt_sidebar-pj"]').length>0){
		console.debug('click pj ');
		$sub.css('display', '');
		//$sub.hide();
		$('.sidebar-menu.nt_sidebar-pj').css({'display':''});
		public_vars.$sidebarMainMenu.addClass('collapsed');
		$('.sidebar-menu.nt_sidebar-pj .sidebar-menu-inner').css('left', '80px');
		ps_destroy();
		
	}else{
		if(!($li.parent('.main-menu').closest('.sidebar-menu').hasClass('nt_sidebar-pj'))){
			console.debug('click others');
			
			
			$('.sidebar-menu.nt_sidebar-pj').css({'display':'none'});
			public_vars.$sidebarMainMenu.removeClass('collapsed');
						
			$('.sidebar-menu.nt_sidebar-pj .sidebar-menu-inner').css({'left':'340px','position':'fixed'});
			ps_destroy();
		}
	}
...
{
	public_vars.$mainFooter.add( public_vars.$mainContent ).add( public_vars.$sidebarMainMenu ).attr('style', '');
	public_vars.$sidebarMenu.siblings('.nt_sidebar-pj').css('background','#003D3D');
