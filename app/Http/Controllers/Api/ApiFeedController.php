...
class ApiFeedController extends Controller
{
    ...
    public function getFeedList()
    {
        ...
        foreach($resultData as $key => $rs){
            ...
            $photoData = $this->apiFeedService->setProfilePhotoList($rs);
    ...
    public function getMyFeedList()
    {
        ...
        $resultData = $this->apiFeedService->getMyFeedList($params);
        $feedTotal = $this->apiFeedService->getMyFeedTotal($params);
        $photoList = [];
    ...
    public function getFeedView()
    {
        ...
        $resultData2 = $this->apiFeedService->getFeedFile($params,'content');
        $resultData3 = $this->apiFeedService->getFeedFile($params,'music');

        $photoData = $this->apiFeedService->setProfilePhotoList($resultData1[0]);
        ...
        $returnData['response']['music'] = $resultData3;
        ...
        try{
            ...
            if($params['wr_file'] == $params['content_cnt']){
                ...
                if($sub_files != null){
                    ...
                    // 첨부파일 db 등록 및 서버 저장
                    $resultData2 = $this->apiFeedService->setFeedFileUpdate($params,$sub_files,'content');
                }

                if($params['wr_type'] != 'daliy'){
                    if($music_file != null){

                        // 첨부파일 db 등록 및 서버 저장
                        $resultData3 = $this->apiFeedService->setFeedMusicFileUpdate($params,$music_file,'music');
                    }
                }
    ...
    public function feedUpdate()
    {
        ...
        try{
            ...
            $params['music_idx'] = $params['music_idx'] ?? [];
            $params['seconds'] = $params['seconds'] ?? [];

            $main_files = $this->request->file('main_file');
            $sub_files = $this->request->file('sub_file');
            $music_files = $this->request->file('music_file');
            ...
            // 음원파일 db 수정 및 서버 저장
            $resultData3 = $this->apiFeedService->feedMusicFileUpdate($params,$music_files);
