    ...
    public function getCommentList($params) {
        ...
        $resultData = $this->statDB->table('comment')
            ...
            ->select(
                ...
                'comment.cm_open',
                'comment.cm_content',
                DB::raw("(select count(idx) from beat_data where service_name = '".$params['wr_type']."' and service_idx = comment.idx and is_beat = 1) as cm_bit"),
                DB::raw("(select count(idx) from beat_data where service_name = '".$params['wr_type']."' and service_idx = comment.idx and mem_id = {$params['mem_id']} and is_beat = 1) as like_status"),
                DB::raw("(select count(idx) from beat_data where service_name = '".$params['wr_type']."' and service_idx = comment.idx and not_beat != 0) as cm_not_bit"),
                DB::raw("(select count(idx) from beat_data where service_name = '".$params['wr_type']."' and service_idx = comment.idx and is_beat = 1 and beat_data.mem_id = comment.mem_id) as checkedLike"),
                DB::raw("(select count(idx) from beat_data where service_name = '".$params['wr_type']."' and service_idx = comment.idx and not_beat !=0 and beat_data.mem_id = comment.mem_id) as unCheckedLike"),
